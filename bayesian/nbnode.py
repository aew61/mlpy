...
class nbnode(object):
    def __init__(self, label, dependent_labels=[]):
        self._label = label
        ...
        self._truth_table = numpy.zeros((self._rows, self._columns), dtype=float)
    ...
    def index_row(self, dependency_states_dict):
        ...
        for label in self._dependent_labels[::-1]:  # <list>[::-1] creates a shallow copy
            if label.get_label() in dependency_states_dict:
                index += (label.hash(dependency_states_dict[label.get_label()]) * base)
            else:
                index += (label.hash(label.default_state()) * base)
